...
function findLongestWord(str) {
  ...
  const biggestItem = Math.max(...wordsLength);
  const result = words.find((item) => item.length === biggestItem);
  return result;
}
...
function extractNameFromTemplate(value) {
  ...
  const result = fullName[1].concat(' ', fullName[2]);
